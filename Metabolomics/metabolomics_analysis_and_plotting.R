# Plotting and analyzing NMR results
#
...
# (all metabolites and single spent media)
#-----------------------------------------------------------------
...
hm.parameters <- list(RI_zm_ss, 
                      color = col.pal,
                      cellwidth = 15, cellheight = 20, scale = "none",
                      treeheight_row = 25,
                      treeheight_col = 30,
                      fontsize = fontsize, fontsize_row = fontsize,
                      fontsize_col = fontsize,
...
# Rare emergent metabolites are those which only
...
write("Emergent Metabolism\n", file = outputEmergentMetsFileName, append = FALSE, sep = "\t")
write("In this study, we define cases of \"emergent metabolism\" as any metabolic behavior which changes in the presence of another species.", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("This broad definition can be divided into 6 categories:", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("Key: 1 = ASF species only produced metabolite x when grown in fresh media", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("     2 = ASF species only consumed metabolite x when grown in spent media", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("     3 = ASF species switched from producing metabolite x (when grown in fresh media) to consuming metabolite x when grown in spent media", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("     4 = ASF species produced metabolite x only when grown in spent media", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("     5 = ASF species only consumed metabolite x when grown in fresh media", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("     6 = ASF species switched from consuming metabolite x (when grown in fresh media) to producing metabolite x when grown in spent media", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
write("We define \"rare emergent metabolites\" for each ASF species as metabolites which display emergent behavior (any of the 6 categories) in a single condition.", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
...
for(s in species)
{
  ...
  write("Rare Emergent Metabolites:", file = outputEmergentMetsFileName, append = TRUE, sep = "\t")
